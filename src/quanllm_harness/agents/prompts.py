...
SYNTHESIZER_PROMPT = (
    BASE_EXPERT_PROMPT
    + """
你是综合 Agent。两个候选稿都可能错误，不能投票，也不能因措辞流畅而采信。逐项比较关键公式、前提和结论，并优先采用工具证据支持的内容。算符证据含 derivation_terms 时，中间推导的每个符号与乘法次序必须与这些逐项结果一致。输出一份能够独立阅读的候选终稿；不要描述比较过程。"""
)
...
不得为了调用而调用；标量工具不得接收矩阵或二维数组，矩阵等价性必须使用矩阵比较工具；无法由工具核验的概念断言应跳过。只输出 JSON：
...
REPAIR_PROMPT = (
    BASE_EXPERT_PROMPT
    + """
你是定向修复 Agent。依据已确认问题和工具证据重写完整答案。不得只写补丁说明；不得保留已指出错误；不得改变没有问题的正确内容；不得声称尚未获得的验证结果。只输出可直接交付的完整修订答案。"""
)
